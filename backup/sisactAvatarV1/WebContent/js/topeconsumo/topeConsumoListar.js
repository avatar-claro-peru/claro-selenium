...
function load(){
	...
	if(btnNew.obj){
		...
		btnNew.obj.action="topeConsumo.htm";
	...
	if(btnDelete.obj){
		...
		btnDelete.obj.action="topeConsumo.htm";
	...
	function doEdit(id){
		...
		if(objForm.id.value != ''){
			...
			objForm.action = "topeConsumo.htm";
			objForm.submit();
		} 
		else{
			alert("Debe seleccionar un Tope de consumo");
		}
	}
	...
	function doDelete(id){		
		...
		objForm.id.value = id;
		if(objForm.id.value != ''){
			confirmSweet('Se solicito la eliminacion de un Tope de consumo, \n desea continuar ?',"doDelete","topeConsumo.htm");
			...
			swal("Debe seleccionar un Tope de consumo.");
		}
	...
	var v = new validator('formTopeConsumoListar', a_fields, o_config);
