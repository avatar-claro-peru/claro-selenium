...
<script type="text/javascript" src="js/perfil/perfilEdit.js"></script>
</head>
<body>
<form  method="post" name="formPerfilEdit">
<input type="hidden" name="metodo" />
<input type="hidden" name="id" value="${perfil.id}"/>
<br> 
	<h3 style="padding: 20px 20px 20px 50px;">
	<c:if test="${empty perfil.id}">
		Crear Perfil
	</c:if>
	...
	<table class="searchLayout">
		<tr> 
			...
			<td><input  id="t_codigoParm" type="text" name="id" value="${perfil.id}" maxlength="10" size="10" 
			disabled="disabled"
			> </td> 
		</tr>
		<tr> 
			<td id="t_loginParm" style="text-align: right;">Nombre </td> 
			<td><input type="text" id="t_loginParm" name="nombre" value="${perfil.nombre}" maxlength="50" size="30"> </td> 
		</tr>
		<tr> 
			<td id="t_loginParm" style="text-align: right;">Descripcion </td> 
			<td><input type="text" id="t_loginParm" name="descripcion" value="${perfil.descripcion}" maxlength="50" size="30"> </td> 
		</tr>		
		<tr> 
			<td id="t_valorParm" style="text-align: right;">Estado</td> 
			<td>
				<select id="cboEstadoParametro" name="estado">
					<option value="1" <c:if test="${perfil.estado=='1'}">selected="selected"</c:if>>Activo</option>
					<option value="0" <c:if test="${perfil.estado=='0'}">selected="selected"</c:if>>Inactivo</option>
				</select>
			</td> 
		</tr>
	</table>
